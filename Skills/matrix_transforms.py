...
def make_scale_matrix(scale_x=1.0, scale_y=1.0):
    ...
    if np.isclose(scale_x, 0.0) or np.isclose(scale_y, 0.0):
        raise ValueError(
            f"Scale values should be non_zero {scale_x}, {scale_y}")
...
def make_matrix_from_sequence(seq):
    ...
    for s in seq:
        ...
        if "translate" in s["type"]:
            ...
            raise ValueError(
                f"Expected one of translate, scale, rotate, got {s['type']}")
...
# YOUR CODE HERE
    x_vec = np.linalg.norm(mat @ np.array([1, 0, 0]))
    y_vec = np.linalg.norm(mat @ np.array([0, 1, 0]))
    return x_vec, y_vec
...
# YOUR CODE HERE
    # https://math.stackexchange.com/questions/1022682/how-do-i-prove-that-a-matrix-is-a-rotation-matrix
    # return True
    # return np.array_equal(np.linalg.inv(mat), mat.T)
    x_axis, y_axis = get_axes_from_matrix(mat)
    xunit = np.linalg.norm(x_axis)
    yunit = np.linalg.norm(y_axis)

    if np.isclose(xunit, 1) and np.isclose(yunit, 1) and np.isclose(np.dot(x_axis, y_axis), 0):
        return True
    return False

    print(mat @ mat.T)
    if np.array_equal(mat @ mat.T, np.identity(3)) and abs(np.linalg.det(mat)) == 1:
        print("ERER")
        return True
    return False
...
# YOUR CODE HERE
    x_axis, y_axis = get_axes_from_matrix(mat)
    angle = np.arccos(np.dot(x_axis, y_axis) /
                      (np.linalg.norm(x_axis) * np.linalg.norm(y_axis)))
    return angle < 1.5708
...
def test_scale_matrix():
    ...
    for sx, sy in sx_sys:
        ...
        if not np.isclose(sx_back, sx) or not np.isclose(sy_back, sy):
            raise ValueError(
                f"Matrix extraction not correct {mat}, ({sx}, {sy}), ({sx_back}, {sy_back})")
...
def test_rotation_matrix():
    ...
    for theta in thetas:
        ...
        if np.count_nonzero(np.isclose(mat_ident, np.identity(3))) != 9:
            raise ValueError(
                "Rotation matrix inverse not transpose {theta}, {mat}")
        ...
        if not np.isclose(theta, theta_back):
            raise ValueError(
                f"Matrix not built correctly {mat}, {theta}, {theta_back}")
...
def test_checks(b_print=False):
    ...
    if check_is_rotation(mat_not_orthonormal, b_print):
        raise ValueError(
            f"Matrix {mat_not_orthonormal} is not orthonormal, should fail length check")
    ...
    if check_is_rotation(mat_not_orthonormal, b_print):
        raise ValueError(
            f"Matrix {mat_not_orthonormal} is not orthonormal, should fail orthogonal check")

    mat_mirrored = make_scale_matrix(2.0, -1.2)
    if not check_is_mirrored(mat_mirrored):
        raise ValueError(
            f"Matrix {mat_mirrored} is mirrored, should return True")

    mat_skewed = make_scale_matrix(2.0, 1.2) @ make_rotation_matrix(0.25)
    if check_preserves_angles(mat_skewed):
        raise ValueError(
            f"Matrix {mat_skewed} does NOT preserve angles, should return False")

    mat_is_ok = make_rotation_matrix(np.pi/3.0) @ make_translation_matrix(
        0.2, -0.3) @ make_rotation_matrix(-np.pi/2.0) @ make_scale_matrix(0.2, 2.0)
    if check_is_mirrored(mat_is_ok):
        raise ValueError(
            f"Matrix {mat_is_ok} is NOT mirrored, should return False")
    if not check_preserves_angles(mat_is_ok):
        raise ValueError(
            f"Matrix {mat_is_ok} is angle-preserving, should return True")
    if check_is_rotation(mat_is_ok):
        raise ValueError(
            f"Matrix {mat_is_ok} is NOT orthonormal, should return False")

    mat_is_ortho = make_rotation_matrix(
        np.pi/3.0) @ make_rotation_matrix(-np.pi/2.0)
    mat_is_not_ortho = make_rotation_matrix(
        np.pi/3.0) @ make_translation_matrix(0.2, -0.3) @ make_rotation_matrix(-np.pi/2.0)
    if not check_is_rotation(mat_is_ortho):
        raise ValueError(
            f"Matrix {mat_is_ortho} is orthonormal, should return True")
    if check_is_rotation(mat_is_not_ortho):
        raise ValueError(
            f"Matrix {mat_is_not_ortho} is NOT orthonormal, should return False")
...
def plot_zigzag(axs, mat):
    ...
    # Draw moved zigzag
    pts_moved = mat @ pts
    axs.plot(pts_moved[0, :], pts_moved[1, :],
...
# YOUR CODE HERE
    seq_rot_trans = [{"type": "scale", "sx": .5, "sy": 2},
                     {"type": "rotate", "theta": np.pi/4.0},
                     {"type": "translate", "dx": 1, "dy": 2}
                     ]
...
# YOUR CODE HERE
    seq_mirrored = [{"type": "translate", "dx": 1, "dy": 2},
                    {"type": "rotate", "theta": np.pi/4.0}]
...
# YOUR CODE HERE
    pts_circle_x_up = make_rotation_matrix(-np.pi/2.0) @ pts_circle
    pts_zigzag_x_up = make_rotation_matrix(-np.pi/2.0) @ pts_zigzag

    pts_circle_lower_left_origin = make_translation_matrix(1, 1) @ pts_circle
    pts_zigzag_lower_left_origin = make_translation_matrix(1, 1) @ pts_zigzag

    pts_circle_x_up_lower_left_origin = make_translation_matrix(1, 1) @ make_rotation_matrix(
        -np.pi/2.0) @ pts_circle
    pts_zigzag_x_up_lower_left_origin = make_translation_matrix(1, 1) @ make_rotation_matrix(
        -np.pi/2.0) @ pts_zigzag
    ...
    list_pts_circle = [pts_circle_lower_left_origin,
                       pts_circle_x_up, pts_circle_x_up_lower_left_origin]
    list_pts_zigzag = [pts_zigzag_lower_left_origin,
                       pts_zigzag_x_up, pts_zigzag_x_up_lower_left_origin]
    for i, (n, c, z) in enumerate(zip(list_names, list_pts_circle, list_pts_zigzag)):
        ...
        pts_moved = mat @ z
        axs[i+1].plot(pts_moved[0, :], pts_moved[1, :],
                      linestyle='dashed', color='grey')
