...
# YOUR CODE HERE
    obj_wedge["Matrix seq"].append(
        mt.make_scale_dict(base_height/2, base_width/2))
    obj_wedge["Matrix seq"].append(mt.make_rotation_dict(np.pi/2))
    obj_wedge["Matrix seq"].append(mt.make_translation_dict(0, base_height/2))
...
# YOUR CODE HERE
    obj_square["Matrix seq"].append(
        mt.make_scale_dict(arm_length/2, arm_height/2))
    obj_square["Matrix seq"].append(
        mt.make_translation_dict(arm_length/2, 0))

    # Force recalculation of matrix
    obj_square["Matrix"] = mt.make_matrix_from_sequence(
        obj_square["Matrix seq"])
...
# YOUR CODE HERE
    obj_square["Matrix seq"].append(
        mt.make_scale_dict((palm_width/2)/10, palm_width/2))
...
# YOUR CODE HERE
    obj_wedge["Matrix seq"].append(
        mt.make_scale_dict(finger_size[0]/2, finger_size[1]/2))
    if b_is_top:
        obj_wedge["Matrix seq"].append(
            mt.make_translation_dict(finger_size[0]/2, palm_width/2))
    else:
        obj_wedge["Matrix seq"].append(
            mt.make_translation_dict(finger_size[0]/2, -palm_width/2))
...
def get_matrix_base(base_link):
    ...
    sx, sy = mt.get_sx_sy_from_matrix(base_link["Matrix"])
    theta = mt.get_theta_from_matrix(base_link["Matrix"])
    r = mt.make_rotation_matrix(theta)
    s = mt.make_scale_matrix(sx, sy) @ r @ [1, 0, 0]
    t = mt.make_translation_matrix(s[0], s[1])
    matrix = t @ r
    return matrix
...
def get_matrix_link(arm_link):
    ...
    #    Reminder: mt.get_xx_from_matrix is helpful here...
    #    Rotate first, then translate
    sx, sy = mt.get_sx_sy_from_matrix(arm_link["Matrix"])
    theta = mt.get_theta_from_matrix(arm_link["Matrix"])
    r = get_rotation_link(arm_link)
    s = r @ mt.make_scale_matrix(sx, sy) @ [2, 0, 0]
    t = mt.make_translation_matrix(s[0], s[1])
    matrix = t @ r
    return matrix
# YOUR CODE HERE
...
# YOUR CODE HERE
    x, y = mt.get_dx_dy_from_matrix(finger["Matrix"])
    t = np.linalg.inv(mt.make_translation_matrix(x, y))
    r = mt.make_rotation_matrix(finger["Angle"])
    t2 = mt.make_translation_matrix(x, y)
    matrix = t2 @ r @ t
    return matrix
...
def get_gripper_location(arm_with_angles):
    ...
    # The distance out along the x axis that we'll call the "grasp" point
    grasp_dist = gripper[0]["Grasp"]
...
# YOUR CODE HERE
    ...
    mats = get_matrices_all_links(arm_with_angles)
    wr = get_rotation_link(gripper[0])
    dist = mt.make_translation_matrix(grasp_dist, 0)
    gr = mats[-1] @ wr @ dist
    dx, dy = mt.get_dx_dy_from_matrix(gr)
    return (dx, dy)
...
# YOUR CODE HERE
    ...
    mats = get_matrices_all_links(arm_with_angles)
    wr = get_rotation_link(gripper[0])
    dist = mt.make_translation_matrix(1, 0)
    gr = mats[-1] @ wr @ dist
    dx, dy = mt.get_dx_dy_from_matrix(gr)
    return (dx, dy)
...
def plot_arm_components(axs, arm, b_with_angles=False):
    ...
    axs_x, axs_y = mt.get_axes_from_matrix(
        mt.make_rotation_matrix(palm["Angle"]))
    # Scale by palm width
...
def plot_complete_arm(axs, arm, matrices):
    ...
    for i, component in enumerate(arm[:-1]):
        ...
        plot_object_in_world_coord_system(
            axs, component, matrices[i] @ get_rotation_link(component))

    gripper = arm[-1]
    # The palm
    wrist_rotation = get_rotation_link(gripper[0])
    plot_object_in_world_coord_system(
        axs, gripper[0], matrices[-1] @ wrist_rotation)

    for finger in gripper[1:3]:
        plot_object_in_world_coord_system(
            axs, finger, matrices[-1] @ wrist_rotation @ get_matrix_finger(finger))
    ...
    axs.plot([x, x + vx * gripper[0]["Grasp"]], [y, y + vy *
             gripper[0]["Grasp"]], '-g', label="Grasp orientation")
...
if __name__ == '__main__':
    ...
    arm_geometry = create_arm_geometry(
        base_size_param, link_sizes_param, palm_width_param, finger_size_param)
    if len(arm_geometry) != 5:
        print(
            "Wrong number of components, should be 5, got {len(arm_geometry)}")
    if len(arm_geometry[-1]) != 3:
        print(
            "Wrong number of gripper components, should be 3, got {len(arm_geometry[-1])}")
    ...
    fig, axs = plt.subplots(1, len(arm_geometry),
                            figsize=(4 * len(arm_geometry), 4))
    ...
    angles_check_fingers = [np.pi/2, -np.pi/4, -
                            3.0 * np.pi/4, [0.0, np.pi/4.0, -np.pi/4.0]]
    angles_check_wrist = [np.pi/2, -np.pi/4, -
                          3.0 * np.pi/4, [np.pi/3.0, 0.0, 0.0]]
    angles_check = [np.pi/2, -np.pi/4, -3.0 *
                    np.pi/4, [np.pi/3.0, np.pi/4.0, -np.pi/4.0]]
    ...
    angles_gripper_check = [np.pi/6.0, -np.pi/4, 1.5 *
                            np.pi/4, [np.pi/3.0, -np.pi/8.0, np.pi/6.0]]
